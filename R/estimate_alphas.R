...
estimate_alphas <- function(data, ret, n_portfolios = 10, lag = 6) {
  ...
  compute_alpha <- function(x, formula) {
    x %>%
      select(portfolio, MCALDT, ret = {{ret}}, MKTRF, SMB, HML, RMW, CMA, MOM) %>%
      group_by(portfolio) %>%
      nest(data = c(MCALDT, ret, MKTRF)) %>%
      mutate(model = map(x, ~lm(formula, data = .)),
  ...
  average_ret <- data %>%
    select(portfolio, MCALDT, ret = {{ret}}) %>%
    group_by(portfolio) %>%
    nest(data = c(MCALDT, ret)) %>%
  ...
  out <- rbind(average_ret, capm_alpha, ff3_alpha, ff5_alpha, ff6_alpha)
  colnames(out) <-c(as.character(seq(1, n_portfolios, 1)), str_c(n_portfolios, "-1"))
  rownames(out) <- c("Excess Return", "t-Stat",
                     "CAPM Alpha", "t-Stat",
                     "FF3 Alpha", "t-Stat",
                     "FF5 Alpha", "t-Stat",
                     "FF6 Alpha", "t-Stat")
